...
ProcessGroupNCCL::~ProcessGroupNCCL() {
  ...
  // group name can register a fresh comm. Safe to call when nothing is
  // registered (no-op).
  c10d::symmetric_memory::NCCLDevCommManager::get(
...
std::shared_ptr<NCCLComm> ProcessGroupNCCL::initNCCLComm(
  ...
  if (it != inInitializationCommMap_.end()) {
    ...
    // for. Multi-device-per-rank symm_mem is not supported by this path.
    {
      ...
      if (mgr.get_comm_if_exists(getGroupUid()) == nullptr) {
